...
class _SubsessionsScreenState extends State<SubsessionsScreen> {
  ...
  Future<List<Subsession>> _listSubsessions(
    ...
    try {
      return await api.listSubsessions(session.sessionId as String);
    } catch (_) {
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (ctx, scrollController) => ListView(
          ...
          children: [
            ...
            _detailLine(ctx, 'Runs',
                '${sub.runs ?? 0}${sub.maxRuns != null ? ' / ${sub.maxRuns}' : ''}'),
            if (sub.intervalSeconds != null)
              _detailLine(
                  ctx, 'Interval', _formatInterval(sub.intervalSeconds!)),
            if (sub.nextRunAt != null)
              _detailLine(ctx, 'Next run', _formatFuture(sub.nextRunAt!)),
            _detailLine(
                ctx, 'Last activity', _formatPast(sub.lastActivityAt)),
